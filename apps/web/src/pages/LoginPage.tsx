...
export default function LoginPage() {
  ...
  return (
    <main className="login-page">
      ...
      <BlueCard eyebrow={t('auth.login.eyebrow')} title={t('auth.login.title')}>
        {error && <BlueAlert message={error} />}
        <form className="login-form" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
          <BlueInput label={t('auth.login.username')} value={username} autoComplete="username" onChange={(event) => setUsername(event.target.value)} />
          <BlueInput label={t('auth.login.password')} type="password" value={password} autoComplete="current-password" onChange={(event) => setPassword(event.target.value)} />
          <BlueButton type="submit" disabled={isLoading}>{isLoading ? t('auth.login.submitting') : t('common.actions.signIn')}</BlueButton>
        </form>
      </BlueCard>
    </main>
  );
